  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f4f1eb] text-[#222321] selection:bg-[#b9e7de]">
      ...
      <section id="workspace" className="relative mx-auto max-w-[1360px] px-5 py-20 sm:px-8 lg:px-12 lg:py-28"><div className="absolute right-0 top-20 -z-10 h-80 w-80 bg-[#dcece7] opacity-60 blur-3xl"/><div className="mb-12 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><div className="mb-4 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-[#0f8f86]"><WandSparkles size={14} /> Build your quote</div><h2 className="font-serif text-5xl tracking-[-0.06em] sm:text-6xl">先說你要做什麼。</h2><p className="mt-4 max-w-[520px] text-sm leading-7 text-[#6f6e67]">選一個最接近的行業，描述這次專案。報價智造會先提出一個可編輯的起點。</p></div><div className="flex items-center gap-2 text-[11px] font-medium text-[#8e8b82]"><span className="size-2 rounded-full bg-[#0f8f86]" /> 草稿自動保留在本機</div></div>
        <div className="grid gap-6 lg:grid-cols-[0.83fr_1.17fr]">
          ...
          <div className="relative border border-[#222321]/15 bg-[#e9e5dc] p-4 sm:p-6"><div className="mb-4 flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#8c887d]">Live preview / {industry}</p><h3 className="mt-1 text-xl font-semibold">報價單預覽</h3></div><div className="flex gap-1"><button onClick={() => notifyAction("預覽設定")} className="grid size-8 place-items-center border border-[#222321]/15 bg-[#f4f1eb]" aria-label="更多設定"><MoreHorizontal size={16}/></button></div></div><div className="min-h-[490px] border border-[#222321]/10 bg-[#fffdf8] p-5 shadow-[0_12px_35px_rgba(47,53,48,0.12)] sm:p-8"><div className="flex justify-between border-b border-[#222321]/15 pb-5"><div><div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.15em] text-[#0f8f86]"><BadgeCheck size={13} /> QuoteCraft AI</div><h4 className="font-serif text-3xl tracking-[-0.05em]">專案服務報價單</h4></div><div className="text-right text-[10px] leading-5 text-[#99958a]">QUOTE NO.<br/><strong className="font-mono text-[#222321]">QC-2026-018</strong></div></div><div className="grid grid-cols-2 gap-4 border-b border-[#222321]/10 py-5 text-[11px]"><div><p className="mb-1 text-[#aaa69b]">客戶名稱</p><p className="font-semibold">日常咖啡 Daily Coffee</p></div><div><p className="mb-1 text-[#aaa69b]">專案類型</p><p className="font-semibold">{industry}</p></div></div><div className="py-4">{items.map((item,index)=><div key={`${item.name}-${index}`} className="group grid grid-cols-[26px_1fr_auto] gap-3 border-b border-dashed border-[#222321]/10 py-3 text-[12px]"><span className="font-mono text-[10px] text-[#aaa69b]">0{index+1}</span><div><p className="font-semibold">{item.name}</p><p className="mt-1 text-[10px] text-[#949187]">{item.detail}</p></div><div className="flex items-start gap-2"><span className="font-mono">NT$ {money(item.price)}</span><button className="hidden text-[#aaa69b] group-hover:block" onClick={() => setItems((current) => current.filter((_,i) => i !== index))} aria-label={`刪除${item.name}`}><X size={13}/></button></div></div>)}</div><button onClick={addItem} className="flex items-center gap-1.5 py-3 text-[11px] font-bold text-[#0f8f86] hover:underline"><Plus size={14}/> 新增報價項目</button><div className="mt-3 flex items-end justify-between border-t border-[#222321]/15 pt-5"><div className="max-w-[230px] text-[10px] leading-5 text-[#99958a]">付款條件：簽約 50%，交付尾款 50%。報價有效期限 14 天。</div><div className="text-right"><p className="text-[10px] text-[#99958a]">含稅總額</p><p className="mt-1 font-mono text-2xl font-bold tracking-[-0.06em]">NT$ {money(total)}</p><p className="mt-1 text-[10px] text-[#aaa69b]">未稅小計 NT$ {money(subtotal)} · 稅額 NT$ {money(tax)}</p></div></div></div><div className="mt-4 flex flex-wrap justify-end gap-2"><button onClick={() => notifyAction("已複製報價連結")} className="flex items-center gap-2 border border-[#222321]/15 bg-[#f4f1eb] px-3 py-2 text-[11px] font-bold hover:border-[#0f8f86]"><Copy size={14}/> 複製連結</button><button onClick={() => notifyAction("準備下載 PDF")} className="flex items-center gap-2 bg-[#222321] px-3 py-2 text-[11px] font-bold text-[#f4f1eb] hover:bg-[#0f8f86]"><Download size={14}/> 下載 PDF</button></div></div>
        </div>
      </section>
