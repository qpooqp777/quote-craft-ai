...
export default function Home() {
  ...
  const [items, setItems] = useState(() => starterQuoteItems.map((item) => ({ ...item, quantity: 1, unitPrice: item.price })));
  ...
  const [companyAddress, setCompanyAddress] = useState("台北市大安區信義路四段 100 號 8 樓");
  const [taxId, setTaxId] = useState("90123456");
  const [contactInfo, setContactInfo] = useState("hello@hillstudio.tw · 02-2700-1234");
  const [brandColor, setBrandColor] = useState("#0F8F86");
  ...
  const subtotal = useMemo(() => items.reduce((sum, item) => sum + (item.quantity ?? 1) * (item.unitPrice ?? item.price), 0), [items]);
  ...
  const addItem = () => {
    setItems((current) => [...current, { id: `custom-${Date.now()}`, industryId: "custom", name: "追加服務項目", detail: "點擊此處編輯說明", price: 3000, unitPrice: 3000, quantity: 1, unit: "式" }]);
  ...
  const updateItem = (index: number, field: "name" | "detail" | "unitPrice" | "quantity", value: string) => {
    setItems((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: field === "name" || field === "detail" ? value : Math.max(0, Number(value) || 0), price: field === "unitPrice" ? Math.max(0, Number(value) || 0) : item.price } : item));
  };

  const draftKey = "quotecraft-current-draft";
  const saveDraft = () => {
    localStorage.setItem(draftKey, JSON.stringify({ industry, prompt, items, customerName, quoteDate, paymentTerms, companyAddress, taxId, contactInfo, brandColor, discountRate, taxRate, logoUrl, activeStyleId }));
    toast.success("草稿已儲存", { description: "下次開啟這個工作區時，可以繼續編輯。" });
  };

  const loadDraft = () => {
    try {
      const draft = JSON.parse(localStorage.getItem(draftKey) ?? "null");
      if (!draft) { toast("目前沒有可載入的草稿"); return; }
      if (draft.industry) setIndustry(draft.industry); if (draft.prompt) setPrompt(draft.prompt); if (draft.items) setItems(draft.items); if (draft.customerName) setCustomerName(draft.customerName); if (draft.quoteDate) setQuoteDate(draft.quoteDate); if (draft.paymentTerms) setPaymentTerms(draft.paymentTerms); if (draft.companyAddress) setCompanyAddress(draft.companyAddress); if (draft.taxId) setTaxId(draft.taxId); if (draft.contactInfo) setContactInfo(draft.contactInfo); if (draft.brandColor) setBrandColor(draft.brandColor); if (typeof draft.discountRate === "number") setDiscountRate(draft.discountRate); if (typeof draft.taxRate === "number") setTaxRate(draft.taxRate); if (draft.logoUrl) setLogoUrl(draft.logoUrl); if (draft.activeStyleId) setActiveStyleId(draft.activeStyleId);
      toast.success("已載入上次草稿");
    } catch { toast.error("草稿載入失敗", { description: "資料格式可能已損壞。" }); }
  };

  const clearDraft = () => { localStorage.removeItem(draftKey); toast("已清除暫存草稿"); };
    ...
    <main className="min-h-screen overflow-hidden bg-[#f4f1eb] text-[#222321] selection:bg-[#b9e7de]">
      ...
      <section id="workspace" className="relative mx-auto max-w-[1360px] px-5 py-20 sm:px-8 lg:px-12 lg:py-28"><div className="absolute right-0 top-20 -z-10 h-80 w-80 bg-[#dcece7] opacity-60 blur-3xl"/><div className="mb-12 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><div className="mb-4 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-[#0f8f86]"><WandSparkles size={14} /> Build your quote</div><h2 className="font-serif text-5xl tracking-[-0.06em] sm:text-6xl">先說你要做什麼。</h2><p className="mt-4 max-w-[520px] text-sm leading-7 text-[#6f6e67]">選一個最接近的行業，描述這次專案。報價智造會先提出一個可編輯的起點。</p></div><div className="flex items-center gap-2 text-[11px] font-medium text-[#8e8b82]"><span className="size-2 rounded-full bg-[#0f8f86]" /> 草稿自動保留在本機</div></div>
        <div className="mb-6 flex flex-col gap-3 border-y border-[#222321]/12 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="proof-tag"><span className="size-1.5 rounded-full bg-[#0f8f86]" /> 版面風格</span><span className="text-sm font-semibold">{activeStyle.name}</span><span className="hidden text-[11px] text-[#8e8b82] sm:inline">{activeStyle.description}</span></div><div className="relative"><button onClick={() => setStyleOpen(!styleOpen)} className="flex items-center gap-2 border border-[#222321]/18 bg-[#fffdf8] px-3 py-2 text-[11px] font-bold shadow-[2px_2px_0_#222321] transition-all hover:-translate-y-0.5 active:translate-y-0">切換 20 種 AI 版面 <ChevronDown size={14} /></button>{styleOpen && <div className="absolute right-0 top-11 z-20 grid max-h-80 w-[min(88vw,390px)] grid-cols-2 gap-px overflow-y-auto border border-[#222321]/15 bg-[#222321]/15 p-px shadow-[0_16px_32px_rgba(47,53,48,.16)] sm:grid-cols-3"><div className="col-span-2 flex items-center justify-between border-b border-[#222321]/12 bg-[#f4f1eb] px-3 py-2 sm:col-span-3"><span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#8e8b82]">{favoriteStyleIds.length ? `最愛風格 ${favoriteStyleIds.length} 組` : "選一個適合客戶的版面"}</span><button onClick={() => setStyleOpen(false)} className="text-[#8e8b82] hover:text-[#222321]" aria-label="關閉風格選單"><X size={14}/></button></div>{quoteStyles.map((style) => <div key={style.id} className={`relative bg-[#fffdf8] p-2 text-left transition-colors hover:bg-[#e7eee8] ${activeStyleId === style.id ? "ring-2 ring-inset ring-[#0f8f86]" : ""}`}><button onClick={() => applyStyle(style.id)} className="block w-full text-left"><div className="mb-2 h-12 overflow-hidden border border-[#222321]/10" style={{ backgroundColor: style.paper }}><div className="h-2.5" style={{ backgroundColor: style.accent }} /><div className="space-y-1 p-2"><span className="block h-1 w-3/4" style={{ backgroundColor: style.ink }} /><span className="block h-1 w-1/2" style={{ backgroundColor: style.muted }} /><span className="block h-1 w-full" style={{ backgroundColor: style.border }} /><span className="block h-1 w-2/3" style={{ backgroundColor: style.border }} /></div></div><span className="block truncate text-[10px] font-bold">{style.name}</span><span className="mt-1 block text-[9px] leading-4 text-[#8e8b82]">{style.category}</span></button><button onClick={() => toggleFavorite(style.id)} className={`absolute right-2 top-2 grid size-6 place-items-center border bg-[#fffdf8]/90 ${favoriteStyleIds.includes(style.id) ? "border-[#0f8f86] text-[#0f8f86]" : "border-[#222321]/10 text-[#aaa69b]"}`} aria-label={favoriteStyleIds.includes(style.id) ? `取消收藏${style.name}` : `收藏${style.name}`}><Heart size={12} fill={favoriteStyleIds.includes(style.id) ? "currentColor" : "none"}/></button></div>)}</div>}</div></div><div className="grid gap-6 lg:grid-cols-[0.83fr_1.17fr]">
          <div className="border border-[#222321]/15 bg-[#fffdf8] p-6 shadow-[0_16px_50px_rgba(47,53,48,0.08)] sm:p-8"><div className="mb-7 flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#aaa69b]">Step 01 / 02</p><h3 className="mt-2 text-xl font-semibold">告訴我們你的專案</h3></div><div className="flex items-center gap-2"><span className="hidden font-mono text-[11px] text-[#0f8f86] sm:inline">{generated ? "READY" : "DRAFT"}</span><button onClick={saveDraft} className="border border-[#222321]/15 bg-[#f4f1eb] px-2.5 py-2 text-[10px] font-bold hover:border-[#0f8f86]">儲存草稿</button><button onClick={loadDraft} className="border border-[#222321]/15 bg-[#f4f1eb] px-2.5 py-2 text-[10px] font-bold hover:border-[#0f8f86]">載入</button></div></div><label className="mb-3 block text-[12px] font-bold">你的行業</label><div className="relative mb-7"><select value={industry} onChange={(e) => setIndustry(e.target.value)} className="w-full appearance-none border border-[#222321]/20 bg-[#f4f1eb] px-4 py-3 text-sm outline-none transition-colors focus:border-[#0f8f86]"><option>室內設計</option><option>行銷顧問</option><option>攝影服務</option><option>工程承包</option><option>自由接案</option></select><ChevronDown className="pointer-events-none absolute right-4 top-3.5" size={16} /></div><label className="mb-3 block text-[12px] font-bold">用一句話描述需求</label><textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={6} className="w-full resize-none border border-[#222321]/20 bg-[#f4f1eb] px-4 py-3 text-sm leading-6 outline-none transition-colors focus:border-[#0f8f86]" placeholder="例如：幫客戶規劃一套..."/><div className="mt-4 flex items-start gap-2 text-[11px] leading-5 text-[#858279]"><Lightbulb size={15} className="mt-0.5 shrink-0 text-[#0f8f86]" /> 描述越接近你平常的工作語言，建議的品項就會越貼近實際。</div><button onClick={generateQuote} className="mt-7 flex w-full items-center justify-center gap-2 bg-[#0f8f86] px-4 py-3.5 text-sm font-bold text-white shadow-[4px_4px_0_#222321] transition-all hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98]">{generated ? "重新整理這份報價" : "生成我的報價草稿"} <Sparkles size={16} /></button></div>
          <div className="relative border p-4 sm:p-6" style={{ backgroundColor: activeStyle.surface, borderColor: activeStyle.border }}><div className="mb-4 flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#8c887d]">Live preview / {industry}</p><h3 className="mt-1 text-xl font-semibold">報價單預覽</h3></div><div className="flex gap-1"><button onClick={() => notifyAction("預覽設定")} className="grid size-8 place-items-center border border-[#222321]/15 bg-[#f4f1eb]" aria-label="更多設定"><MoreHorizontal size={16}/></button></div></div><div className="mb-3 grid gap-3 border border-[#222321]/12 bg-[#f4f1eb] p-4 sm:grid-cols-2 lg:grid-cols-4"><label className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#88857B]">客戶名稱<input value={customerName} onChange={(event) => setCustomerName(event.target.value)} className="mt-2 w-full border-b border-[#222321]/20 bg-transparent py-1 text-[12px] font-semibold outline-none focus:border-[#0f8f86]" /></label><label className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#88857B]">報價日期<input type="date" value={quoteDate} onChange={(event) => setQuoteDate(event.target.value)} className="mt-2 w-full border-b border-[#222321]/20 bg-transparent py-1 text-[12px] font-semibold outline-none focus:border-[#0f8f86]" /></label><label className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#88857B]">付款條件<input value={paymentTerms} onChange={(event) => setPaymentTerms(event.target.value)} className="mt-2 w-full border-b border-[#222321]/20 bg-transparent py-1 text-[12px] font-semibold outline-none focus:border-[#0f8f86]" /></label><label className="flex items-end gap-2 text-[10px] font-bold uppercase tracking-[0.1em] text-[#88857B]"><span className="flex-1">品牌 Logo<input type="file" accept="image/png,image/jpeg,image/svg+xml" onChange={handleLogoUpload} className="mt-2 block w-full text-[10px] font-normal normal-case tracking-normal file:mr-2 file:border-0 file:bg-[#222321] file:px-2 file:py-1 file:text-[10px] file:font-bold file:text-white" /></span>{logoUrl ? <img src={logoUrl} alt="已上傳品牌 Logo" className="size-9 object-contain" /> : <span className="grid size-9 place-items-center border border-dashed border-[#222321]/25 text-[9px]">LOGO</span>}</label></div><div className="mb-4 grid gap-3 border border-[#222321]/12 bg-[#fffdf8] p-4 sm:grid-cols-2 lg:grid-cols-4"><label className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#88857B]">公司地址<input value={companyAddress} onChange={(event) => setCompanyAddress(event.target.value)} className="mt-2 w-full border-b border-[#222321]/20 bg-transparent py-1 text-[12px] outline-none focus:border-[#0f8f86]" /></label><label className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#88857B]">統一編號<input value={taxId} onChange={(event) => setTaxId(event.target.value)} inputMode="numeric" className="mt-2 w-full border-b border-[#222321]/20 bg-transparent py-1 text-[12px] outline-none focus:border-[#0f8f86]" /></label><label className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#88857B]">聯絡資訊<input value={contactInfo} onChange={(event) => setContactInfo(event.target.value)} className="mt-2 w-full border-b border-[#222321]/20 bg-transparent py-1 text-[12px] outline-none focus:border-[#0f8f86]" /></label><label className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#88857B]">品牌主色<div className="mt-2 flex items-center gap-2"><input type="color" value={brandColor} onChange={(event) => setBrandColor(event.target.value)} className="h-8 w-10 border-0 bg-transparent p-0" /><input value={brandColor} onChange={(event) => setBrandColor(event.target.value)} className="w-full border-b border-[#222321]/20 bg-transparent py-1 font-mono text-[11px] uppercase outline-none focus:border-[#0f8f86]" /></div></label></div><div ref={quotePreviewRef} className="min-h-[490px] border p-5 sm:p-8" style={{ backgroundColor: activeStyle.paper, borderColor: activeStyle.border, borderRadius: activeStyle.radius, boxShadow: activeStyle.shadow, color: activeStyle.ink }}><div className="mb-3 flex items-center justify-between"><span className="proof-tag" style={{ backgroundColor: `${brandColor}20`, color: brandColor }}><span className="size-1.5 rounded-full" style={{ backgroundColor: brandColor }} /> AI 版面已套用</span><span className="font-mono text-[9px] text-[#99958a]">{activeStyle.id.toUpperCase()}</span></div><div className="flex justify-between border-b border-[#222321]/15 pb-5"><div className="flex items-start gap-3"><div className="grid size-10 shrink-0 place-items-center border border-dashed text-[9px]" style={{ borderColor: brandColor, color: brandColor }}>{logoUrl ? <img src={logoUrl} alt="品牌 Logo" className="size-full object-contain" /> : "LOGO"}</div><div><div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.15em]" style={{ color: brandColor }}><BadgeCheck size={13} /> QuoteCraft AI</div><h4 className="font-serif text-3xl tracking-[-0.05em]">專案服務報價單</h4></div></div><div className="text-right text-[10px] leading-5 text-[#99958a]">QUOTE NO.<br/><strong className="font-mono text-[#222321]">QC-2026-018</strong><br/><span>{quoteDate}</span></div></div><div className="grid grid-cols-2 gap-4 border-b border-[#222321]/10 py-5 text-[11px]"><div><p className="mb-1 text-[#aaa69b]">客戶名稱</p><p className="font-semibold">{customerName || "未填寫客戶名稱"}</p></div><div><p className="mb-1 text-[#aaa69b]">專案類型</p><p className="font-semibold">{industry}</p></div></div><div className="grid gap-2 border-b border-[#222321]/10 py-4 text-[10px] text-[#858279] sm:grid-cols-3"><p><span className="block uppercase tracking-[0.12em] text-[#aaa69b]">公司地址</span><span className="mt-1 block text-[#4B4A44]">{companyAddress || "未填寫"}</span></p><p><span className="block uppercase tracking-[0.12em] text-[#aaa69b]">統一編號</span><span className="mt-1 block text-[#4B4A44]">{taxId || "未填寫"}</span></p><p><span className="block uppercase tracking-[0.12em] text-[#aaa69b]">聯絡資訊</span><span className="mt-1 block text-[#4B4A44]">{contactInfo || "未填寫"}</span></p></div><div className="py-4">{items.map((item,index)=><div key={`${item.id}-${index}`} className="group grid grid-cols-[26px_1fr_auto] gap-3 border-b border-dashed py-3 text-[12px]" style={{ borderColor: activeStyle.border }}><span className="font-mono text-[10px] text-[#aaa69b]">0{index+1}</span><div className="min-w-0"><input aria-label={`編輯第 ${index + 1} 項名稱`} value={item.name} onChange={(event) => updateItem(index, "name", event.target.value)} className="w-full border-b border-transparent bg-transparent px-0 py-0 font-semibold outline-none transition-colors focus:border-current"/><input aria-label={`編輯第 ${index + 1} 項說明`} value={item.detail} onChange={(event) => updateItem(index, "detail", event.target.value)} className="mt-1 w-full border-b border-transparent bg-transparent px-0 py-0 text-[10px] text-[#949187] outline-none transition-colors focus:border-current"/></div><div className="flex items-start gap-2"><label className="flex items-center gap-1 font-mono text-[10px] text-[#77736A]">×<input aria-label={`編輯第 ${index + 1} 項數量`} value={item.quantity ?? 1} onChange={(event) => updateItem(index, "quantity", event.target.value)} type="number" min="0" className="w-8 bg-transparent text-right outline-none"/><span>NT$</span><input aria-label={`編輯第 ${index + 1} 項單價`} value={item.unitPrice ?? item.price} onChange={(event) => updateItem(index, "unitPrice", event.target.value)} type="number" min="0" className="w-20 bg-transparent text-right font-mono outline-none"/></label><span className="hidden min-w-[72px] text-right font-mono text-[11px] sm:inline">{money((item.quantity ?? 1) * (item.unitPrice ?? item.price))}</span><button className="hidden text-[#aaa69b] group-hover:block" onClick={() => setItems((current) => current.filter((_,i) => i !== index))} aria-label={`刪除${item.name}`}><X size={13}/></button></div></div>)}</div><div className="flex flex-wrap items-center justify-between gap-3"><button onClick={addItem} className="flex items-center gap-1.5 py-3 text-[11px] font-bold text-[#0f8f86] hover:underline"><Plus size={14}/> 新增報價項目</button><span className="text-[10px] text-[#99958a]">點擊文字與金額即可編輯</span></div><div className="mt-3 grid gap-5 border-t border-[#222321]/15 pt-5 sm:grid-cols-[1fr_auto]"><div className="space-y-3"><div className="flex flex-wrap items-center gap-3 text-[10px] text-[#77736A]"><label className="flex items-center gap-2">折扣 <input value={discountRate} onChange={(event) => setDiscountRate(Math.min(100, Math.max(0, Number(event.target.value) || 0)))} type="number" min="0" max="100" className="w-14 border-b border-[#222321]/20 bg-transparent py-1 text-right font-mono outline-none" /> %</label><label className="flex items-center gap-2">稅率 <input value={taxRate} onChange={(event) => setTaxRate(Math.min(100, Math.max(0, Number(event.target.value) || 0)))} type="number" min="0" max="100" className="w-14 border-b border-[#222321]/20 bg-transparent py-1 text-right font-mono outline-none" /> %</label></div><div className="max-w-[280px] text-[10px] leading-5 text-[#99958a]">付款條件：{paymentTerms || "尚未設定"}。報價有效期限 14 天。</div></div><div className="min-w-[180px] text-right text-[10px] text-[#99958a]"><p>項目小計 <span className="font-mono text-[#4B4A44]">NT$ {money(subtotal)}</span></p><p className="mt-1">折扣 <span className="font-mono text-[#B45D4E]">− NT$ {money(discountAmount)}</span></p><p className="mt-1">稅額 <span className="font-mono text-[#4B4A44]">NT$ {money(tax)}</span></p><p className="mt-2 border-t border-[#222321]/15 pt-2 text-[#4B4A44]">含稅總額</p><p className="mt-1 font-mono text-2xl font-bold tracking-[-0.06em]" style={{ color: brandColor }}>NT$ {money(total)}</p></div></div></div><div className="mt-4 flex flex-wrap justify-end gap-2"><button onClick={() => notifyAction("已複製報價連結")} className="flex items-center gap-2 border border-[#222321]/15 bg-[#f4f1eb] px-3 py-2 text-[11px] font-bold hover:border-[#0f8f86]"><Copy size={14}/> 複製連結</button><button onClick={exportImage} className="flex items-center gap-2 border border-[#222321]/15 bg-[#f4f1eb] px-3 py-2 text-[11px] font-bold hover:border-[#0f8f86]"><ImageDown size={14}/> 高清 PNG</button><button onClick={exportPdf} className="flex items-center gap-2 bg-[#222321] px-3 py-2 text-[11px] font-bold text-[#f4f1eb] hover:bg-[#0f8f86]"><FileDown size={14}/> 下載 PDF</button></div></div>
        </div>
      </section>
